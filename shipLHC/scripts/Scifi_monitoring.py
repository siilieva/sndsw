...
# summary canvases of the median, maximum and overflow of signals
       ...
       ut.bookCanvas(h,detector+'signalsSummary'+xi,' ',1024,768,1,2)
       ...
       graph_list = [signal_medians, signal_maxima]
...
# end of the summary QDC shifter canvases
       ...
       for canvas in [detector+'hitmaps'+xi,detector+'signal'+xi,detector+'mult'+xi]:
           self.M.h[canvas].Update()
           if xi!='': self.M.myPrint(self.M.h[canvas],"Scifi-"+canvas,subdir='scifi/expert/'+xi)
           else:     self.M.myPrint(self.M.h[canvas],"Scifi-"+canvas,subdir='scifi/expert')
...
# analysis and plots 
   def Plot(self):
       ...
       for xi in self.xing:
           ...
           ut.bookCanvas(h,tname,"track position first state",512,1024,1,2)
           h[tname].cd(1)
